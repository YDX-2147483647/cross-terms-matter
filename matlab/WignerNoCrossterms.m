...
% xlabel('时间 (秒)');
% ylabel('频率 (Hz)');
% title('一个信号是确定信号一个信号是随机信号的WVD');
% xlim([0.15 0.35]);
% ylim([20 130]);
% colorbar;

% %% 一个信号是确定信号，另一个信号是随机信号——利用Monte Carlo仿真去做
% Nlist=[10,100,500,1000,2000];  % 随机生成的样本数量分别进行
% WVD_MonteCarlo=cell(length(Nlist),2);   % 储存仿真结果
% Cross_maxvalue=zeros(1,length(Nlist));  
% tf=cell(2,1);
% 
% % 以交叉项时频中心为原点框定矩形区域
% f_cross=(f0+f1)/2;
...
% df_cross=25;
% dt_cross=0.01;
% f_crossmin=(f_cross-df_cross)/fs;
% f_crossmax=(f_cross+df_cross)/fs;
% t_crossmin=(t_cross-dt_cross)/T;
% t_crossmax=(t_cross+dt_cross)/T;
% 
...
%     % 求取交叉项的最大值
%     % 框定的矩形区域：
%     f_crossst=round(f_crossmin*length(fx));
%     f_crossend=round(f_crossmax*length(fx));
%     t_crossst=round(t_crossmin*length(tx));
%     t_crossend=round(t_crossmax*length(tx));
%     Crossterms=sum(f_crossst:f_crossend,t_crossst:t_crossend);
%     Cross_maxvalue(1,i)=max(abs(Crossterms), [], "all");
%     
%     figure;
%     color_range = max(abs(sum), [], "all");
%     imagesc(tx, fx, sum, [-1, 1] * color_range);
...
% tf{2,1}=fx;
% % 保存Monte Carlo仿真结果
% % save("WVD_MonteCarlo.mat","WVD_MonteCarlo");
% % save("Cross_maxvalue.mat","Cross_maxvalue");
% % save("tf.mat","tf");

%% 再次加载数据进行绘制图像
tf=load("tf.mat").tf;           % 时间轴和频率轴
WVD_MonteCarlo=load("WVD_MonteCarlo.mat").WVD_MonteCarlo;   % 样本数量和WVD
Cross_maxvalue=load("Cross_maxvalue.mat").Cross_maxvalue;   % 交叉项最大值
Nl=length(WVD_MonteCarlo);
Samplenum=zeros(1,Nl);
for nl=1:Nl
    Samplenum(1,nl)=WVD_MonteCarlo{nl,1};
...
% 绘制交叉项最大值与样本数量曲线
figure;
p1=plot(log10(Samplenum),log10(Cross_maxvalue));
