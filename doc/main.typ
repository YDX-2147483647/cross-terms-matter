...
Wigner--Ville分布（Wigner--Ville distribution, WVD）是一种双线性形式的时频分布，这意味着WVD会有交叉项存在。本文首先研究了随机信号的演变谱，通过构造包含两个信号分量且两个信号具有均匀随机相位来证明随机信号的WVD可以没有交叉项，并做了Monte Carlo仿真实验。之后，基于互WVD的平移性质讨论了包含两个信号分量的确定信号WVD中交叉项的时域理解、频域理解、几何特征以及其可以被用于检测弱信号。然后，通过提取桌子振动产生的双频信号，分析了WVD如何显现到时域。Wigner函数在1932年被提出，用于描述量子态在相空间中的分布，而后被Ville引入到信号分析中。因此，对于WVD，量子物理与信号之间也许存在着某种对应关系。最后，我们给出了二者之间的翻译词典。
= 介绍
